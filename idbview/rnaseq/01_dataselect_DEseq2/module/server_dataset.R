...
datasetServer <- function(id) {
  moduleServer(
    ...
    function(input, output, session) {
      ...
      observe({
        updateSelectInput(session, "choose_gse",
          choices = paste(dataset()$gse, dataset()$type, sep = "_"),
      ...
      observeEvent(input$show_gse, {
        ...
        updateSelectInput(session, "choose_condition",
          choices = unique(gse()$condition),
          selected = unique(gse()$condition)
        )
        updateSelectInput(session, "choose_cluster",
          choices = unique(gse()$cluster),
          selected = unique(gse()$cluster)
        )
        updateSelectInput(session, "choose_sample",
          choices = unique(gse()$sample),
          selected = unique(gse()$sample)
        )
        updateSelectInput(session, "choose_gender",
          choices = unique(gse()$gender),
              ...
              if (T) {
                output$download_gene_csv <- downloadHandler(
                  ...
                  content = function(file) {
                    write.csv(gse_exp(), file, row.names = T)
                  }
                )
                output$download_gene_rds <- downloadHandler(
                  filename = paste(input$choose_gse, "data.rds", sep = "_"),
                  content = function(file) {
                    saveRDS(gse_exp(), file)
                  }
                )
              }

              observeEvent(input$submit_data, {
                saveRDS(mydata(), file = "../data/select_deseq2_myexp.RDS")
                saveRDS(mygroup(), file = "../data/select_deseq2_mygroup.RDS")
              })
      ...
      observeEvent(input$remove_data, {
        file.remove("../data/select_deseq2_myexp.RDS")
        file.remove("../data/select_deseq2_mygroup.RDS")
      })
    }
  )
}

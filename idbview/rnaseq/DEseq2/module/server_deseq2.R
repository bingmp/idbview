    ...
    function(input, output, session) {
      ...
      observeEvent(input$show, {
        ...
        logi_group0 <- 'try-error' %in% class(
          try( readRDS('../data/select_deseq2_mygroup.RDS'),silent=T) )
        logi_exp <- 'try-error' %in% class( 
          try( readRDS('../data/select_deseq2_myexp.RDS'),silent=T) )
        ...
        if( logi_group0 ){
          ...
          group0 <- get_df("../data/select_deseq2_mygroup.RDS")
          exp <- get_df("../data/select_deseq2_myexp.RDS")

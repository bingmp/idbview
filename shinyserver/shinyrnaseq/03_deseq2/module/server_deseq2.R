  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      observeEvent(input$show, {
        ...
        logi_group0 <- 'try-error' %in% class(
          try( readRDS('~/ShinyApps/shinyserver/shinyrnaseq/data/select_deseq2_mygroup.RDS'),silent=T) )
        logi_exp <- 'try-error' %in% class( 
          try( readRDS('~/ShinyApps/shinyserver/shinyrnaseq/data/select_deseq2_myexp.RDS'),silent=T) )
        ...
        if( logi_group0 ){
          ...
          group0 <- get_df("~/ShinyApps/shinyserver/shinyrnaseq/data/select_deseq2_mygroup.RDS")
          exp <- get_df("~/ShinyApps/shinyserver/shinyrnaseq/data/select_deseq2_myexp.RDS")
          
          output$exp <- DT::renderDataTable( exp() ) 
          output$group <- DT::renderDataTable( group0() ) 
          group <- reactive({
            group <- group0()
          ...
          observeEvent(input$submitDEG, { 
            ...
            exp2 <- reactive({
              ...
              dds <- reactive({
                ...
                rownames(colData) <- as.character(group$gsm )  # 设置行名为样本名信息
                ...
                dds <- DESeq(dds,parallel = T) # 3.2 DEseq2  # 差异分析
